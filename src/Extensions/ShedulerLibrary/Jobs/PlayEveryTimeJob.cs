...
namespace ShedulerLibrary.Jobs
{
    using Trigger.NET;

    /// <summary>
    /// Само собственно задание которое выполняется кроном по расписанию
    /// Это проигрывает WAV файл _через_опрделённые_ промежутки времени
    /// </summary>
    public class PlayEveryTimeJob : IJob
    {
        public void Execute(JobContext context)
        {
#if DEBUG
            var dt = DateTime.Now;
            Trace.WriteLine($" ----------- Execute PlayEveryTimeJob, DateTieme: {dt} ------------------");
#endif
            var player = context.Parameter as IPlayWavFile;
            player?.Play();
        }
    }
}
